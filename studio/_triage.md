...
Direction is a meta-tag (scope: `account` → `region` → `market`, + off-substrate **assets** the research-grounded outlier lane covers); all current rows are `bottom_up` (no top-down market piece grounded yet). The off-substrate **showcase** reads (Galveston corridor, Red Dog) are capability demos, not portfolio holdings — read their verdict as "diligence / pilot," not "act on our book."
...
| Red Dog Mine (Teck/NANA) | asset · bottom_up · **OFF-substrate** | Arctic climate risk (ice-logistics window + permafrost) | directional–medium | **severe** (single-point-of-failure; Norilsk ~$2B geometry) | Arctic ~4× warming; the window already moves the P&L | **WATCH (high)** — off-substrate **showcase**; the named gaps ARE the pilot questions | [red_dog_mine/arctic_climate_risk.md](red_dog_mine/arctic_climate_risk.md) §1 |
