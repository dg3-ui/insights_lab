# 07 - Discovery Spec: `find_methodology` / `get_methodology`

> **Status**: v0 spec — **design, MCP-ready not built**, 2026-06-05.
>
> **Purpose**: define the contract + matching algorithm for the discovery layer that turns *"a BizDev user searches a topic"* into *"the right skill is found and loaded,"* over the registry built from each package's taxonomy. Resolves `06_architecture.md` §13 #5(b) (top-N selection) and #5(c) (tie-break) and adopts an alias map for #5(a); **recommends** (does not resolve) #2 (location).
>
> **Companions**: `06_architecture.md` §8 (where discovery sits), `resources/README.md` (the registry this indexes), `resources/weather_and_climate/el_nino_enso/SKILL.md` (the first skill).
...
SkillIndexRecord {
  ...
  family:             string    // taxonomy.family   (enum, docs/02)
  ...
  status:             string     // = resource.yml.identity.status: draft | active | deprecated
                                  //   only "active" (eval-passed) is indexed by default (06 §9 gate, §13 #7)
}
```

**Source-of-truth note**: the matchable `description` comes from **`SKILL.md`** (it is literally the Claude-Skill discovery field — *what it does and when to use it*); `taxonomy` + `identity` come from `resource.yml`. The publish step asserts `folder == slug` and stamps `version` (`06` §13 #4/#6).

**Publish-eligibility**: the publish step indexes a skill **only after its eval suite passes** (`06` §9). `status` is copied from `resource.yml.identity.status`; a `draft` / eval-failing skill is **not** added to the served index by default (it can still be fetched with `include_deprecated=true`). This is the discovery analog of the §9 gate — discovery must not surface a method that never passed its gate.
...
It returns **ranked candidates — it does not auto-load**. The session (via `get_methodology`) loads the chosen slug. This resolves `06` §13 #5(b) toward **top-N, never silent auto-bind**.
...
| **draft / not-eval-passed skill** | excluded by default; publish refuses to index it until its eval suite passes (§3, `06` §9); surfaced only with `include_deprecated=true` |
| **deprecated skill** | excluded by default; surfaced only with `include_deprecated=true` |

## 7. Synonym / Alias Map (resolves `06` §13 #5a, pragmatically)
...
It lives in the registry as **`resources/aliases.yml`**, loaded/merged at publish into the index. **Publish asserts** every alias's canonical key is a real `drivers` value present somewhere in the registry — a dangling alias **fails the publish loud**, not silently at query time (mirroring the `folder == slug` assertion in §3 / `06` §13 #6). The deeper "full controlled vocabulary vs. fuzzy embedding match" choice stays open (`06` §13 #5a); this spec adopts the **alias map** as the pragmatic middle for the current scale.

## 8. `get_methodology` — Contract

```text
get_methodology({ slug, file? })   // slug = resource.yml.identity.slug — the canonical key (06 §8)
→ {
    slug, name, title, version,    // version echoed for publish coherence (06 §13 #4)
...
## 9. Where It Lives (recommendation for `06` §13 #2)
...
- **`06` §13 #2** (where discovery lives) — recommended MCP tool; contract is location-agnostic.
- **`06` §13 #5a** (controlled vocabulary vs. fuzzy) — this spec adopts an alias map as the pragmatic middle; revisit a full controlled vocabulary / embedding match at ~15–20 skills.
- **Discovery eval** — discovery needs its own eval ("does query X return skill Y in the top-K?"); folds into the eval harness (`06` §11).

---

**See also**: `06_architecture.md` §8, `resources/README.md`, `resources/weather_and_climate/el_nino_enso/SKILL.md`.
