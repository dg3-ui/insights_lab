...
**Outputs and the studio layer.** The three outputs are **blog · report · email** (blog generic, report scoped to a portfolio/client/region, email a subset of either). The output half is the **studio layer** (`../studio/README.md`): a confidence foundation (`method/confidence_model.md`), a subject-keyed render layer, and an internal triage board — where `direction` (top_down/bottom_up) is a **meta-tag**, not a folder. Origination is **studio-first for the artifact** (the insight is gated upstream) and **bottom_up before top_down**; the *form envelope* is owned by `resources/_style/output_contracts.md`. Bottom-up is now realized (`../studio/brookfield_standard_solar/` + the Galveston corridor); the architecture is `plans/2026-06-15_output_layer_architecture.md`. This doc points, it does not re-enumerate.
