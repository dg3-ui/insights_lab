...
**The activation boundary (standing, not just a V0 constraint).** The lab produces the **gated insight** and its **rendering** (the studio brief, the deliverable); **actual send / outreach / distribution stays post-gate and human-in-the-loop — no send automation, ever.** This is durable, independent of V0's MCP-integration status: even once tools and rendering are wired, a human reviews the rendered artifact before it reaches a named account. This is the boundary that `../studio/README.md`, the `/render` command, `resources/_style/output_contracts.md` §4, and the studio briefs all dereference.

**Business-development handoff.** After render, client/customer-ready artifacts move through the Google Drive operating model in `client_delivery_flow.md`: `client-centric/` for bottom-up account/client folders and `climate-centric/` for top-down phenomenon folders. That layer organizes profiles, reports, vlogs, presentations, and email templates for senior-owned follow-up. It does **not** ground claims; it routes and frames already gated work.

**Outputs — baseline first, studio selective.** The three outputs are **blog · report · email** (blog generic, report scoped to a portfolio/client/region, email a subset of either). The **baseline** path needs no studio: a gated insight (the methodology package + MCP produced it) renders to those via **`/render`** — *even MCP + a package alone yields something useful*; the *form envelope* is `resources/_style/output_contracts.md`, confidence is **posture on the face, the cap grade internal** (`method/confidence_model.md`). **`studio/`** (`../studio/README.md`) is the **selective** amplification lane on top — a subject-keyed render brief (`direction` a meta-tag, not a folder) feeding an internal triage board, for the pieces that must land; **most outputs skip it**. studio-first applies *within* that lane (the artifact originates there; the insight is always gated upstream; bottom_up before top_down). Bottom-up is realized (`../studio/brookfield_standard_solar/` + the Galveston corridor); the architecture is `plans/2026-06-15_output_layer_architecture.md`. This doc points, it does not re-enumerate.
...
V0 is complete when a new contributor can explain the deliverable from `README.md` + this doc; the projection runs a manual test; the test yields one applied-insight draft; every material claim has a source ref or an explicit external dependency; confidence + caveats + actor relevance are present (confidence is three axes — posture on the rendered face, the cap grade internal — `method/confidence_model.md`); and failures are categorized (method / MCP-tool / external-data / orchestration gaps). The current status ladder is `status/capabilities.md`.

---

**See also**: `architecture.md` (the 3-pillar overview + the 4 layers), `client_delivery_flow.md` (the business-development handoff after render), `method/analysis_families.md` (the families, orthogonal to these buckets), `method/confidence_model.md` (the 3-axis confidence foundation), `../studio/README.md` (the Layer-3 render/triage architecture this activates), `resources/_style/output_contracts.md` (the three-output form envelope this scopes), `principles.md` P2 (content downstream of validated insight), `status/capabilities.md` (where V0 stands).
