...
- **Content is downstream of validated insight**: insight → gate → render, never the reverse (`principles.md` P2). The output half is the **studio layer** (`studio/README.md`): gated insight → `studio/<subject>/<phenomenon>.md` → `/render` (blog/report/email + grounded charts/maps). studio originates the OUTPUT artifact (studio-first; bottom_up before top_down), never the insight.
