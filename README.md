...
V0 is validated (Test Run 001 PASS) and the methodology layer is hardened with principles (`08`), the MCP ledger (`09`), the command toolchain (`10`), and agent files; the project brief (`00`) is now the single visual overview. The active work is the **layered-reference restructure** — shared cross-cutting layers (`_principles` · `_style` · `_craft` · populated `_reference`), typed output contracts (direction × audience × format), and the canonical 5-criteria rubric — executed one phase at a time per **`docs/plans/2026-06-11_layered_reference_v1.md`**. The "second ENSO test run done the new way" is that plan's Phase 6 (the composed revalidation). The publish step, discovery tool, and eval harness stay paused until that process loop has been exercised at least once.
