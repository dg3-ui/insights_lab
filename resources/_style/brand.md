...
Intent rules (format-agnostic): **every figure is bordered with its caption below it** (description under the image — never above, never floating); the caption names source + `as_of`; boxes are **lifted** (rounded corners + a subtle shadow — the exact radius/shadow are a Part-B delivery detail); **generous whitespace** (density lives in tables, not cramped text). **Confidence (public) — three axes, `../../docs/method/confidence_model.md`:** the summary's calibration cue and the footer's descriptor state the *posture* ("a forward, directional read"), never a pejorative "LOW" badge; alongside it the cue may carry the **event likelihood** (the sourced number, "NOAA: 63% very-strong El Niño") and, where a firming forecast exists, a small **settledness bar** (`▰▰▰▱▱`, "firms as the season resolves" — a forward indicator, never "we're unsure"). The **cap grade, the materiality band, and the triage verdict (act/watch/noise) never render** on a public/client face (render-internal, independent of repo visibility — `confidence_model.md` §7); an internal/expert report *may* keep the explicit cap line where the decider wants the label (`output_contracts.md` §3 · `confidence_model.md` §3a) — that audience-conditional carve-out is the one exception, and it never extends to the materiality band or the triage verdict. The caveat box carries a **phenomenon** caveat (what's resolving in the world) **+ a forward door** (the deeper InfraSure layer — "talk to the team"), never a *capability-confession* ("we run no model") or a "what we are NOT saying" negation list (`output_contracts.md` §3 · `../../docs/principles.md` P7). **Kicker:** reader-facing topic ("ENSO" · "California Solar"), never the internal bucket name. **Sources / `as_of` register:** renders in *italic* (muted) — the citation apparatus, set visually apart from body copy; IDs and tool-call tokens inside it keep the mono face.
...
4. **Pagination, whitespace & visual connectedness (binds on every paginated format — DOCX · PDF · slides).** A paginated export must **not end on a near-empty page** — an orphaned tail of a few lines under a sea of whitespace reads unfinished. Either tighten spacing / figure sizing so the tail lands on the previous page, **or** balance content so the **final page is at least ~half full**. Just as important, **nothing related gets severed by a page break**: a heading must never sit alone at a page foot with its body on the next page, a figure must never split from its caption, and a callout box or table must not break mid-element. Mechanically (DOCX): set **keep-with-next on every heading *and* on any label that introduces a table/figure/list**, and **keep-together** on figures (image+caption), callout boxes, and table rows. This is the "things stay connected and look intentional" check, and it only shows once paginated: **render to PDF and eyeball every page break, not just the last page** (orphaned headings, split figures, and stranded labels are the usual offenders). It generalizes to a PDF or a deck.
