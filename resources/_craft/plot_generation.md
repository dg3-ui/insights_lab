...
- **Numbers are a directional screen (P7, on the figure itself).** A capacity / CF / quantitative figure (and any exact-looking number it carries) gets a **standing note** — in or beside the caption, or as one framing line that governs the figures that follow — saying the values are **approximate and directional, a fleet-level screen rather than a site or portfolio model**, and that **asset- and portfolio-level detail comes from the InfraSure team**. Two reasons it is mandatory here, not optional polish: (1) an exact-looking chart invites a point-precision question it cannot survive (`../_principles/voice.md` §3 — false precision), so the note manages the expectation the plot creates; (2) it is the **forward door** (`../../docs/principles.md` P7) rendered in figure form — it deflects "what's the exact number for plant X" into the real deeper capability instead of leaving the chart to imply a rigor it never claimed. Word it so it is true whether the series is observed history or modeled (e.g. "approximate, directional; for asset/portfolio detail contact the InfraSure team"). Keep it light: one framing line plus a short caption tag beats repeating a paragraph under every figure.
