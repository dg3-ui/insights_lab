...
| 4 | **accuracy** (with calibration — **"smartfulness"**) | Is every material claim grounded with a `source_ref` + vintage, confidence capped at the weakest input, blocked claims refused — **and is the altitude right**: no false precision (an exact, unmodeled number invites the question mark it cannot survive) and no generic mush (a claim so hedged it says nothing). Calibration *is* the defining clause of accuracy here, not a sixth criterion. | `../../docs/method/resource_standard.md` Confidence Rules + Blocked Claim Examples · the package's `blocked_claims` · `../../docs/method/analysis_families.md` Q4–Q5 |
