...
> **Status**: v0 registry, updated 2026-06-29 — **15 resources across 3 live domains** (weather_and_climate · hazard · commercial); the Registry table below is current. Historical-context layer (`historical_context.md`) exists across all hazard packages as cited frames/external evidence, never substrate grounding.
>
...
│   ├── tornado_solar_wind/    # tornado × solar/wind/substation assets
│   ├── flooding_multi_asset/  # flood inundation × thermal/solar/wind/grid assets
│   ├── ice_storm_transmission/ # ice storm × transmission/wind/delivery-dependent assets
│   ├── sea_level_rise_surge/  # sea level rise + surge × coastal thermal/nuclear/grid
│   └── lightning_multi_asset/ # lightning × wind/solar/electrical-plant assets
...
| `hazard` ✅ | physical peril | SPC / NHC / FEMA / USGS / NIFC / NOAA, hazards news, footprint | **hail_solar** ✅ · **hurricane_high_wind_wind** ✅ · **wildfire** draft · **extreme_cold_winter_storm** draft · **hurricane_coastal_flood** draft · **riverine_flood** draft · **tornado_solar_wind** draft · **flooding_multi_asset** draft · **ice_storm_transmission** draft · **sea_level_rise_surge** draft · **lightning_multi_asset** draft |
...
| [tornado_solar_wind](hazard/tornado_solar_wind/) | `hazard` | Exposure (+ Event-Translation) | owner · investor · lender · developer | manual test 001 PASS · status `draft` · grounding `model-not-wired` (Tornado-Alley corridor + catastrophic-but-localized mechanism grounded; narrow-footprint per-plant strike probability, sub-plant geometry, and $ layers unwired — anchor: Traverse Wind EIA 63479, Custer Co OK) |
| [flooding_multi_asset](hazard/flooding_multi_asset/) | `hazard` | Exposure (+ Event-Translation) | owner · investor · lender · developer | manual test 001 PASS · status `draft` · grounding `model-not-wired` (water-sited thermal siting + cooling-intake/switchyard mechanisms grounded; site elevation, inundation depth, and $ layers unwired; `search_news` no `category=` param; news returned `[]` — anchor: Sabine EIA 3459, Orange Co TX) |
| [ice_storm_transmission](hazard/ice_storm_transmission/) | `hazard` | Exposure (+ Event-Translation) | owner · investor · lender · developer | manual test 001 PASS · status `draft` · grounding `model-not-wired` (ice-belt geography + delivery-infrastructure framing grounded; conductor/tower ratings, line geometry, and $ layers unwired — anchor: Wagon Wheel Wind EIA 69224, Logan Co OK) |
| [sea_level_rise_surge](hazard/sea_level_rise_surge/) | `hazard` | Exposure (+ Event-Translation) | owner · investor · lender · developer | manual test 001 PASS · status `draft` · grounding `model-not-wired` (tidewater siting + chronic/acute split grounded; site elevation/freeboard, inundation year/depth, surge probability, and $ layers unwired — anchor: South Texas Project EIA 6251, Matagorda Co TX) |
| [lightning_multi_asset](hazard/lightning_multi_asset/) | `hazard` | Exposure (+ Event-Translation) | owner · investor · lender · developer | manual test 001 PASS · status `draft` · grounding `model-not-wired` (flash-density geography + direct-strike/induced-surge distinction grounded; LPS-status field, per-strike component damage record, and $ layers unwired — anchor: Echo River Solar EIA 62490, Suwannee Co FL) |
